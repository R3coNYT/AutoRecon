...
THEOF
    ...
    if ! command -v nikto &>/dev/null; then
        log "Installing nikto (web scanner)"
        if [ "$PLATFORM" = "linux" ]; then
    ...
    if ! command -v whatweb &>/dev/null; then
        log "Installing whatweb (web tech fingerprinting)"
        if [ "$PLATFORM" = "linux" ]; then
            $SUDO apt-get install -y whatweb 2>/dev/null && ok "whatweb installed" || warn "whatweb install failed"
        else
            brew install whatweb 2>/dev/null && ok "whatweb install failed (optional)"
        fi
    else
    ...
    if ! command -v wfuzz &>/dev/null; then
        log "Installing wfuzz (web fuzzer)"
        if [ "$PLATFORM" = "linux" ]; then
    ...
    if ! command -v smbmap &>/dev/null; then
        log "Installing smbmap (SMB share enumeration)"
        if [ "$PLATFORM" = "linux" ]; then
            $SUDO apt-get install -y smbmap 2>/dev/null && ok "smbmap installed" || warn "smbmap install failed"
        else
            warn "smbmap: install manually on macOS (git clone https://github.com/ShawnDEvans/smbmap)"
        fi
    else
    ...
    if ! command -v amass &>/dev/null; then
        log "Installing amass (subdomain enumeration)"
        if [ "$PLATFORM" = "linux" ]; then
            if $SUDO apt-get install -y amass 2>/dev/null; then
                ...
                warn "amass install failed — install Go first or run: apt install amass"
            fi
        else
    ...
    if ! command -v enum4linux-ng &>/dev/null; then
        log "Installing enum4linux-ng (enhanced SMB/LDAP enum)"
        if [ "$PLATFORM" = "linux" ]; then
...
E4LEOF
                $SUDO chmod +x /usr/local/bin/enum4linux-ng && ok "enum4linux-ng installed from GitHub" || \
                    warn "enum4linux-ng install failed — clone manually: git clone https://github.com/cddmp/enum4linux-ng.git"
            fi
        else
            warn "enum4linux-ng: Linux only"
        fi
    else
...
LDDEOF
    ...
    if ! command -v feroxbuster &>/dev/null; then
        log "Installing feroxbuster (fast directory brute-force)"
        if [ "$PLATFORM" = "linux" ]; then
            if $SUDO apt-get install -y feroxbuster 2>/dev/null; then
                ...
                if [ -n "$_fb_url" ]; then
                    ...
                    warn "feroxbuster: could not fetch latest release URL"
                fi
            fi
        else
    ...
    if ! command -v testssl.sh &>/dev/null && ! command -v testssl &>/dev/null; then
        log "Installing testssl.sh (TLS/SSL scanner)"
        if [ "$PLATFORM" = "linux" ]; then
            ...
            if $SUDO apt-get install -y sslscan 2>/dev/null; then
                ...
                if git clone --depth 1 https://github.com/rbsec/sslscan.git "$_ss_dir" 2>/dev/null && \
                   make -C "$_ss_dir" static 2>/dev/null; then
                    # Try system-wide install; fall back to ~/.local/bin
